'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }

function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }

var EventBus = /*#__PURE__*/function () {
  function EventBus() {
    _classCallCheck(this, EventBus);
  }

  _createClass(EventBus, [{
    key: "$onMessage",
    value: function $onMessage(type, callBack) {
      if (listenerMap[type]) {
        listenerMap.push(callBack);
      } else {
        listenerMap[type] = [callBack];
      }
    }
  }, {
    key: "$postMessage",
    value: function $postMessage(type) {
      var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

      if (listenerMap[type] !== undefined) {
        listenerMap[type].forEach(function (callBack) {
          return callBack(data);
        });
      } else {
        console.info("EventBus Info: ".concat(type, " not yet $onMessage"));
      }
    }
  }, {
    key: "$removeMessage",
    value: function $removeMessage(type, callBack) {
      if (listenerMap[type] !== undefined) {
        listenerMap[type] = listenerMap[type].filter(fn !== callBack);
      }
    }
  }]);

  return EventBus;
}();

_defineProperty(EventBus, "listenerMap", {});

var _default = new EventBus();

exports["default"] = _default;